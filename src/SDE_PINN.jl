...
eq = Dt(u(t, z1, z2)) ~ α * u(t, z1, z2) +
                        β * u(t, z1, z2) *
                        (√2 *
                         (z1 * cos((1 - 1 / 2) * π * t) + z2 * cos((2 - 1 / 2) * π * t)))
bcs = [u(0, z1, z2) ~ 1.0]
...
for j in 1:num_samples
    for i in 1:num_time_steps
        ...
        predicted_solution_samples[i, j] = first(phi(
            [ts[i], z1_samples[j], z2_samples[j]], res.minimizer))
        truncated_solution_samples[i, j] = truncated_sol(
            u0, ts[i], z1_samples[j], z2_samples[j])
    end
end
